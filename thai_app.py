    ...
    <script>
        ...
        function startRecognition() {{
            if (recognition) {{
                try {{
                    ...
                    btn.style.backgroundColor = "%23CC0000".replace('%23', '#');
        ...
        function saveCurrentTranslation() {{
            ...
            window.parent.postMessage({{
                ...
                value: {{ thai: thai, english: english, category: "USER ADDED", timestamp: Date.now() }}
            }}, '*');
...
if isinstance(component_result, dict) and component_result.get("thai"):
    ...
    ts_val = component_result.get("timestamp", 0)
    
    save_key = f"{thai_val}_{eng_val}_{ts_val}"
    if st.session_state.get("last_saved_key") != save_key:
        try:
            ...
            st.success(f"Successfully appended to Google Sheet: {thai_val} - {eng_val}")
