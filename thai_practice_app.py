...
# Row 1: REVEAL (Centered, half the width of the old 1/3-width column,
# using the SAME column ratio as Row 2 below so the two buttons line up)
_, col_rev, _ = st.columns([2.5, 1, 2.5])
with col_rev:
    if st.button("REVEAL", key="btn_reveal", use_container_width=True):
        st.session_state.reveal = not st.session_state.reveal
        st.rerun()

# Row 2: PHRASE (same [2.5, 1, 2.5] ratio as Row 1 -> vertically aligned with REVEAL)
_, col_phrase, _ = st.columns([2.5, 1, 2.5])
with col_phrase:
    if st.button("PHRASE", key="btn_phrase", use_container_width=True):
        play_thai_audio(current_phrase["thai"])

# Row 3: BACK, RANDOM, NEXT (three equal columns with a real gap between them)
col_back, col_rand, col_next = st.columns([1, 1, 1], gap="medium")
with col_back:
